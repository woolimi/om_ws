...
카메라 경로/해상도/FPS 는 모두 scripts/config.json 에서 읽음.
...
def main():
    ...
    if missing:
        print(f"ERROR: missing keys in config.json: {missing}", file=sys.stderr)
        sys.exit(1)
    ...
    app = TunerApp(
        ...
        cfg["camera_top"],
        cfg["camera_wrist"],
        int(cfg["camera_width"]),
        int(cfg["camera_height"]),
        int(cfg["camera_fps"]),
    )
    app.run()
